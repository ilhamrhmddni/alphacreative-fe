        ...
        {items.map((news, idx) => (
          ...
          >
            <div className="flex items-start justify-between gap-2">
              ...
              {news.photoPath && (
                <span className="inline-flex items-center gap-1 text-[11px] rounded-full border border-slate-200 px-2 py-0.5 text-slate-600">
            ...
            <p className="text-xs text-slate-600">{news.deskripsi}</p>
            {Array.isArray(news.tags) && news.tags.length > 0 && (
              <div className="flex flex-wrap gap-1 text-[11px] text-slate-500">
                {news.tags.map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-[11px] font-medium"
                  >
                    #{tag}
                  </span>
                ))}
          ...
          <Table className="w-full whitespace-nowrap">
            ...
            <TableBody className="divide-y divide-slate-100 bg-white">
              {items.map((news, idx) => (
                <TableRow
                  key={news.id}
                  className="text-sm text-slate-700 transition-colors hover:bg-slate-50 cursor-pointer"
                  onClick={() => onSelect?.(news)}
                >
                  ...
                  <TableCell className="px-4 py-3 align-top text-xs text-slate-500">
                    {news.event?.namaEvent ? (
